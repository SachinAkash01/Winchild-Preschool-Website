...
<head>
<meta charset="utf-8">
<title>Winchild - Edit Student</title>
	...
	<style>
		...
		.imagefit{
			background: url("Images/apply2.jpg") no-repeat center fixed;
...
<body>
	...
	<?php
		...
		$sql1 = "SELECT * FROM `guardian` WHERE `email` = '".$_GET["email"]."';";
	...
	<ul class="breadcrumbs">
		...
		<li class="breadcrumbs_item">
			<a href="studentDetails.php" class="breadcrumbs_link">Student Details</a>
		</li>
		<li class="breadcrumbs_item">
			<a href="#" class="breadcrumbs_link breadcrumbs_link--active">Edit Student</a>
		</li>
	</ul>
	
	<form action="editStudentHandler.php" class="newBody" method="post">
	<div class="form-container">
	<div class="contact-section">
      <div class="contact-form">
        <h2>Student Details</h2>
          <input type="text" name="name" class="text-box" value="<?php echo $row["studentName"]; ?>" required>
          <input type="date" name="date" class="text-box" value="<?php echo $row["DoB"]; ?>" required>
		  <input type="number" name="age" class="text-box" value="<?php echo $row["age"]; ?>" min="1" max="10" required>
		  <select name="program" id="program" class="text-box" required>
			  <option selected><?php echo $row["programName"]; ?></option>
			  <option>Play Group</option>
			  <option>Lower Nursery</option>
			  <option>Upper Nursery</option>
			  <option>After School Care</option>
		  </select>
		  <textarea name="address" rows="5" required><?php echo $row1["address"]; ?></textarea>
		  <div style="text-align: center; color: white; font-size: 18px;">Select Gender: 
			...
			<label class="radio" style="font-size: 18px;">
				<input class="radio-one" type="radio" name="rdoOption" checked>
				<span class="checkmark"></span>
				Male
			</label>
			<label class="radio" style="font-size: 18px;">
				<input class="radio-two" type="radio" name="rdoOption" disabled>
				<span class="checkmark"></span>
				Female
			</label>
			<?php
			...
			?>
			<label class="radio" style="font-size: 18px;">
				<input class="radio-one" type="radio" name="rdoOption" disabled>
				<span class="checkmark"></span>
				Male
			</label>
			<label class="radio" style="font-size: 18px;">
				<input class="radio-two" type="radio" name="rdoOption" checked>
				<span class="checkmark"></span>
				Female
			</label>
			<?php
      ...
      <div class="contact-form">
        <h2>Guardian Details</h2>
          <input type="text" name="gname" class="text-box" value="<?php echo $row1["guardianName"]; ?>" required>
          <input type="email" name="email" class="text-box" value="<?php echo $row1["email"]; ?>" readonly>
		  <input type="text" name="telephone" class="text-box" value="<?php echo $row1["telephone"]; ?>" required>
		  <input type="text" name="occupation" class="text-box" value="<?php echo $row1["occupation"]; ?>" required>
		  <input type="password" name="pwd" class="text-box" value="<?php echo $row1["password"]; ?>" required readonly>
          <br><br><br><br><br><br><br><br><input type="submit" name="btnRegister" id="btnactivity" class="send-btn" value="Edit">
      </div>
	</div>
	</div>
